...
class KEKStore:
    ...
    def unwrap_dek(self, wrapper: KEKWrapper, execution_id: str) -> ExecutionDEK:
        ...
        # Reconstruct AAD - we need to know the DEK key_id
        # In practice, this would be stored alongside the wrapper
        # For now, we derive it after unwrapping
        try:
            dek_bytes = aesgcm.decrypt(nonce, wrapped, None)
        except Exception:
            # Try without AAD for backwards compatibility, then verify
            raise DecryptionError("Failed to unwrap DEK")

        key_id = hashlib.sha256(dek_bytes).hexdigest()[:16]
        return ExecutionDEK(
            execution_id=execution_id,
            key_bytes=dek_bytes,
            key_id=key_id,
