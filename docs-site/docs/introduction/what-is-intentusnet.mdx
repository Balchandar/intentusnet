...
class MyAgent(BaseAgent):
    def __init__(self, router):
        definition = AgentDefinition(
            name="my-agent",
            capabilities=[Capability(intent=IntentRef(name="MyIntent"))]
        )
...
# Register agent - runtime injects the router
runtime.register_agent(MyAgent)

# Send intent via client
client = runtime.client()
response = client.send_intent("MyIntent", {"data": "example"})

# Response includes routing metadata
print(f"Status: {response.status}")
print(f"Agent: {response.metadata.get('agent')}")
print(f"Trace ID: {response.metadata.get('traceId')}")
```
